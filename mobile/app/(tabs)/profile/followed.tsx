...
const api_url_user_followed = API_URL + "/users/followed";
...
export default function FollowedScreen() {
  const [followed, setFollowed] = React.useState<Follow[]>([]);
  ...
  React.useEffect(() => {
    const fetchUserPosts = async () => {
      try {
        setIsLoading(true);
        const response = await apiCall(api_url_user_followed + "/" + userID);
